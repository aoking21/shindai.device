...
extension Array {
    func findIndexInTutorial(includeElement: (Element) -> Bool) -> [Int] {
        var indexArray:[Int] = []
...
class SecondTutorialViewController: UIViewController , UIPickerViewDelegate, UIPickerViewDataSource, UITableViewDelegate, UITableViewDataSource{
    ...
    override func viewDidLoad() {
        ...
        //初期値
        currentBirthYear = "1960"
        currentSex = "その他"
        currentInterest = "無し"
        
        //tableView
        tableView = UITableView(frame: CGRect(x:0,y:200,width:width,height:height - 50))
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 0:
            cell.rightLabel.text = currentBirthYear
        case 1:
            cell.rightLabel.text = currentSex
        case 2:
            cell.rightLabel.text = currentInterest
        default:
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        switch(indexPath.row){
        case 0:
            let index = BirthYearArray.findIndexInTutorial{$0 == cell.rightLabel.text}
            if(index.count != 0){
                pickerView.selectRow(index[0],inComponent:0,animated:false)
            }
        case 1:
            let index = userSexArray.findIndexInTutorial{$0 == cell.rightLabel.text}
            if(index.count != 0){
                pickerView.selectRow(index[0],inComponent:0,animated:false)
            }
        case 2:
            let index = InterestArray.findIndexInTutorial{$0 == cell.rightLabel.text}
            if(index.count != 0){
                pickerView.selectRow(index[0],inComponent:0,animated:false)
            }
        default:
